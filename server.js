...
/*
app.use(cors({
  origin: 'https://veiculos.bragantini.com.br', // Substitua pelo URL do seu frontend
}));
*/
...
sequelize
  ...
  app.get('/', (req, res) =>{
    res.json({message: 'Tudo funcionando Aqui...'})
  })
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    if (driver && (await bcrypt.compare(password, driver.password))) {
      res
        .status(200)
        .json({ message: "Login bem-sucedido", driverId: driver.id });
    } else {
    ...
    res
      .status(500)
      .json({ error: "Erro ao processar login", details: error.message });
  }
});
...
app.post("/api/travels", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao criar Viagem", details: error.message });
  }
});
...
app.put("/api/travels/:id", async (req, res) => {
  try {
    ...
    const travelUpdate = await Travel.findByPk(id);
    if (!travelUpdate) {
...
app.get("/api/vehicles", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar Veículos", details: error.message });
  }
});
...
app.get("/api/travels", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar Viagens", details: error.message });
  }
});
